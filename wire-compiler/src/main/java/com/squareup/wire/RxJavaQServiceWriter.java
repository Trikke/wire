...
public class RxJavaQServiceWriter extends ServiceWriter {
  String requestType;
  String responseType;
  String func1Type;

  public RxJavaQServiceWriter( JavaWriter writer, List<String> options ) {
  ...
  public void emitService(Service service, Set<String> importedTypes) throws IOException {
    ...
    for (Service.Method method : service.getMethods()) {
      ...
      setTypes(method);
      writer.emitAnnotation("POST", "\"/" + method.getName() + "\"");
      writer.beginMethod(responseType, getMethodName(method), EnumSet.noneOf(Modifier.class),
          Arrays.asList("@Body " + requestType, "request"), Arrays.asList( "HttpException" ));
      writer.endMethod();
    }
    writer.endType();
